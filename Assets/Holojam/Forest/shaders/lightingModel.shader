Shader "Custom/lightingModel" {
		...
		struct SurfaceOutputCustom
		{
          ...
          half4 LightingSimpleLambert (SurfaceOutputCustom s, half3 lightDir, half3 viewDir, half atten) {
				...
				float4 c;
				c.rgb = s.Albedo;//+(pow(camNorm,2)*s.Alpha*(1-NdotL));
...
//						   		
				...
				float4 L = lerp(_Color*float4(c.rgb,1.0),float4(c.rgb,1.0),emit.r);//+(pow(camNorm,2)*s.Alpha*(1-NdotL)*.5); 		
				return L;
